    ...
    <div class="p-4 flex-col space-y-4">
        ...
        <x-table>
            <x-slot name="head">
                ...
                <x-table.heading sortable wire:click="sortBy('assignment_id')" :direction="$sortField === 'assignment_id' ? $sortDirection : null" class="w-4/12">Opdracht Naam</x-table.heading>
            ...
            <x-slot name="body">
                @forelse($learningSuboutcomeAssignments as $learningSuboutcomeAssignment)
                    <x-table.row>
                        ...
                        <x-table.cell class="w-1/12">
                            @can('edit learningsuboutcomeassignment')
                                <a href="{{ route('admin.learningsuboutcomeassignments.edit', ['learningsuboutcomeassignment' => $learningSuboutcomeAssignment->id]) }}" class="bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-1 px-2 rounded flex items-center justify-center text-xs w-full">
                                    <i class="fas fa-edit mr-1"></i> Edit
                                </a>
                            @endcan
                        </x-table.cell>
                        <x-table.cell class="w-1/12">
                            @can('delete learningsuboutcomeassignment')
                                <a href="{{ route('admin.learningsuboutcomeassignments.delete', ['learningsuboutcomeassignment' => $learningSuboutcomeAssignment->id]) }}" class="bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-2 rounded flex items-center justify-center text-xs w-full">
                    ...
                    <x-table.row>
                        <x-table.cell colspan="5">
                            <div class="flex justify-center items-center">
                                <span class="font-medium py-8 text-gray-500 text-xl">Geen suboutcome assignments gevonden...</span>
                            </div>
                        </x-table.cell>
                    </x-table.row>
                @endforelse
        ...
        <div class="container max-w-full pb-10 flex justify-between items-center px-3">
            <div class="text-xs text-left">
                <p>
                    Showing {{ $learningSuboutcomeAssignments->firstItem() }} to {{ $learningSuboutcomeAssignments->lastItem() }} of {{ $learningSuboutcomeAssignments->total() }} results
                </p>
            </div>
