    ...
    <div class="p-4 flex-col space-y-4">
            ...
            <x-slot name="body">
                @forelse($learningSuboutcomeLevels as $learningSuboutcomeLevel)
                    <x-table.row>
                        ...
                        <x-table.cell class="w-1/12">
                            @can('edit learningsuboutcomelevel')
                                <a href="{{ route('admin.learningsuboutcomelevels.edit', ['learningsuboutcomelevel' => $learningSuboutcomeLevel->id]) }}" class="bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-1 px-2 rounded flex items-center justify-center text-xs w-full">
                                    <i class="fas fa-edit mr-1"></i> Edit
                                </a>
                            @endcan
                        </x-table.cell>
                        <x-table.cell class="w-1/12">
                            @can('delete learningsuboutcomelevel')
                                <a href="{{ route('admin.learningsuboutcomelevels.delete', ['learningsuboutcomelevel' => $learningSuboutcomeLevel->id]) }}" class="bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-2 rounded flex items-center justify-center text-xs w-full">
                    ...
                    <x-table.row>
                        <x-table.cell colspan="6">
                            <div class="flex justify-center items-center">
                                <span class="font-medium py-8 text-gray-500 text-xl">Geen suboutcome levels gevonden...</span>
                            </div>
                        </x-table.cell>
                    </x-table.row>
                @endforelse
        ...
        <div class="container max-w-full pb-10 flex justify-between items-center px-3">
            <div class="text-xs text-left">
                <p>
                    Showing {{ $learningSuboutcomeLevels->firstItem() }} to {{ $learningSuboutcomeLevels->lastItem() }} of {{ $learningSuboutcomeLevels->total() }} results
                </p>
            </div>
