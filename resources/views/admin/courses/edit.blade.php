...
@section('content')
    <div class="card mt-6">
        <div class="card-header flex flex-row justify-between">
            <h1 class="h6">Course Wijzigen</h1>
        </div>
        ...
        <div class="card-body grid grid-cols-1 gap-6 lg:grid-cols-1">
            <div class="p-4">
                <form id="form" class="shadow-md rounded-lg px-8 pt-6 pb-8 mb-4"
                      action="{{ route('admin.courses.update', ['course' => $course->id]) }}" method="POST">
                    @method('PUT')
                    @csrf
                    <label class="block text-sm">
                        ...
                        focus:outline-none focus:shadow-outline-purple form-input"
                               name="name" value="{{ old('name', $course->name) }}" type="text" required/>
                    </label>
                    <label class="block text-sm">
                        ...
                        <textarea
                            ...
                            id="description" rows="7">{{ old('description', $course->description) }}</textarea>
                    </label>
