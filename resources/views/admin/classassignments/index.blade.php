...
@section('content')
    <div class="card mt-6">
        <div class="card-header flex flex-row justify-between">
            <h1 class="h6">Class Assignment Admin</h1>
        </div>
                ...
                <table class="w-full whitespace-no-wrap">
                    ...
                    @foreach($classAssignments as $classAssignment)
                        <tr class="text-gray-700">
                            <td class="px-4 py-3 text-sm">{{ $classAssignment->classYear->schoolClass->name }}</td>
                            <td class="px-4 py-3 text-sm">{{ $classAssignment->assignment->name }}</td>
                            <td class="px-4 py-3 text-sm">{{ $classAssignment->duedate }}</td>
                            <td class="px-4 py-3 text-sm"><a href="{{ route('admin.classassignments.show', ['classassignment' => $classAssignment->id]) }}">Details</a></td>
                            <td class="px-4 py-3">
                                <div class="flex items-center space-x-4 text-sm">
                                    <a href="{{ route('admin.classassignments.edit', ['classassignment' => $classAssignment->id]) }}">Wijzigen</a>
                                </div>
                            </td>
                            <td>
                                @can('delete classassignment')
                                    <div class="flex items-center space-x-4 text-sm">
                                        <a href="{{ route('admin.classassignments.delete', ['classassignment' => $classAssignment->id]) }}">
                                            Verwijderen</a>
                                    </div>
                                @endcan
                            </td>
