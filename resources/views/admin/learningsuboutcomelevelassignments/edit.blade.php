...
@section('content')
    <div class="card mt-6">
        <div class="card-header flex flex-row justify-between">
            <h1 class="h6">Learning Suboutcome Level Assignment Bewerken</h1>
        </div>
        ...
        <div class="card-body grid grid-cols-1 gap-6 lg:grid-cols-1">
            <div class="p-4">
                <form id="form" class="shadow-md rounded-lg px-8 pt-6 pb-8 mb-4" action="{{ route('admin.lsuboutcomelevelassignments.update', $learningSuboutcomeLevelAssignment->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <label class="block text-sm">
                        ...
                        <select class="bg-gray-200 block rounded w-full p-2 mt-1 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple form-input"
                            ...
                            @foreach($assignments as $assignment)
                                <option value="{{ $assignment->id }}" {{ $learningSuboutcomeLevelAssignment->assignment_id == $assignment->id ? 'selected' : '' }}>
                                    {{ $assignment->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <!-- Learning Suboutcome Level Field -->
                    <label class="block text-sm">
                        <span class="text-gray-700">Learning Suboutcome Level</span>
                        <select class="bg-gray-200 block rounded w-full p-2 mt-1 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple form-input"
                                name="learning_suboutcome_level_id" required>
                            @foreach($learningSuboutcomeLevels as $learningSuboutcomeLevel)
                                <option value="{{ $learningSuboutcomeLevel->id }}" {{ $learningSuboutcomeLevelAssignment->learning_suboutcome_level_id == $learningSuboutcomeLevel->id ? 'selected' : '' }}>
                                    {{ $learningSuboutcomeLevel->learningSuboutcome->name }} ({{ $learningSuboutcomeLevel->learningLevel->name }})
                                </option>
                            @endforeach
                        </select>
                    </label>
