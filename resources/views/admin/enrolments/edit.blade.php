...
@section('content')
    <div class="card mt-6">
        <div class="card-header flex flex-row justify-between">
            <h1 class="h6">Enrolment Wijzigen</h1>
        </div>
        ...
        <div class="card-body grid grid-cols-1 gap-6 lg:grid-cols-1">
            <div class="p-4">
                <form id="form" class="shadow-md rounded-lg px-8 pt-6 pb-8 mb-4"
                      action="{{ route('admin.enrolments.update', ['enrolment' => $enrolment->id]) }}" method="POST">
                    @method('PUT')
                    @csrf
                    <label class="block text-sm">
                        ...
                        <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight
                            focus:outline-none focus:shadow-outline" name="student_id" id="student_id">
                            @foreach($students as $student)
                                <option value="{{ $student->id }}"
                                    @selected($student->id == old('student_id', $enrolment->student_id))>{{ $student->user->name }}</option>
                            @endforeach
                        </select>
                    </label>
                    <label class="block text-sm">
                        ...
                        <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight
                            focus:outline-none focus:shadow-outline" name="crebo_id" id="crebo_id">
                            @foreach($crebos as $crebo)
                                <option value="{{ $crebo->id }}"
                                    @selected($crebo->id == old('crebo_id', $enrolment->crebo_id))>{{ $crebo->name }}</option>
                            @endforeach
                        </select>
                    </label>
                    <label class="block text-sm">
                        ...
                        <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight
                            focus:outline-none focus:shadow-outline" name="cohort_id" id="cohort_id">
                            @foreach($cohorts as $cohort)
                                <option value="{{ $cohort->id }}"
                                    @selected($cohort->id == old('cohort_id', $enrolment->cohort_id))>{{ $cohort->name }}</option>
                            @endforeach
                        </select>
                    </label>
                    <label class="block text-sm">
                        ...
                        <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight
                            focus:outline-none focus:shadow-outline" name="enrolment_status_id" id="enrolment_status_id">
                            @foreach($statuses as $status)
                                <option value="{{ $status->id }}"
                                    @selected($status->id == old('enrolment_status_id', $enrolment->enrolment_status_id))>{{ $status->name }}</option>
                            @endforeach
                        </select>
                    </label>
                    <label class="block text-sm">
                        ...
                        focus:outline-none focus:shadow-outline-purple form-input"
                               name="date" value="{{ old('date', $enrolment->date) }}" type="date" required/>
                    </label>
