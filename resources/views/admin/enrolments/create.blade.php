...
@section('content')
    <div class="card mt-6">
        <div class="card-header flex flex-row justify-between">
            <h1 class="h6">Enrolment Toevoegen</h1>
        </div>
        ...
        <div class="card-body grid grid-cols-1 gap-6 lg:grid-cols-1">
            <div class="p-4">
                <form id="form" class="shadow-md rounded-lg px-8 pt-6 pb-8 mb-4"
                      action="{{ route('admin.enrolments.store') }}" method="POST">
                    @csrf
                        ...
                        <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight
                            ...
                            @foreach($students as $student)
                                <option value="{{ $student->id }}"
                                    @selected($student->id == old('student_id'))>{{ $student->user->name }}</option>
                            @endforeach
                        </select>
                    </label>
                    <label class="block text-sm">
                        ...
                        <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight
                            ...
                            @foreach($crebos as $crebo)
                                <option value="{{ $crebo->id }}"
                                    @selected($crebo->id == old('crebo_id'))>{{ $crebo->name }}</option>
                            @endforeach
                        </select>
                    </label>
                    <label class="block text-sm">
                        ...
                        <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight
                            ...
                            @foreach($cohorts as $cohort)
                                <option value="{{ $cohort->id }}"
                                    @selected($cohort->id == old('cohort_id'))>{{ $cohort->name }}</option>
                            @endforeach
                        </select>
                    </label>
                    <label class="block text-sm">
                        ...
                        <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight
                            ...
                            @foreach($statuses as $status)
                                <option value="{{ $status->id }}"
                                    @selected($status->id == old('enrolment_status_id'))>{{ $status->name }}</option>
                            @endforeach
                        </select>
                    </label>
                    <label class="block text-sm">
