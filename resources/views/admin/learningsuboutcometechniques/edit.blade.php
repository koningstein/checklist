...
@section('content')
    <div class="card mt-6">
        <div class="card-header flex flex-row justify-between">
            <h1 class="h6">Learning Suboutcome Technique Wijzigen</h1>
        </div>
        ...
        <div class="card-body grid grid-cols-1 gap-6 lg:grid-cols-1">
            <div class="p-4">
                <form id="form" class="shadow-md rounded-lg px-8 pt-6 pb-8 mb-4" action="{{ route('admin.learningsuboutcometechniques.update', ['learningsuboutcometechnique' => $learningSuboutcomeTechnique->id]) }}" method="POST">
                    @method('PUT')
                    @csrf
                    <label class="block text-sm">
                        ...
                        <select name="learning_suboutcome_id" class="bg-gray-200 block rounded w-full p-2 mt-1 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple form-select" required>
                            @foreach($learningSuboutcomes as $suboutcome)
                                <option value="{{ $suboutcome->id }}" {{ $learningSuboutcomeTechnique->learning_suboutcome_id == $suboutcome->id ? 'selected' : '' }}>
                                    {{ $suboutcome->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <label class="block text-sm">
                        <span class="text-gray-700">Related Technique</span>
                        <select name="learning_related_technique_id" class="bg-gray-200 block rounded w-full p-2 mt-1 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple form-select" required>
                            @foreach($learningRelatedTechniques as $technique)
                                <option value="{{ $technique->id }}" {{ $learningSuboutcomeTechnique->learning_related_technique_id == $technique->id ? 'selected' : '' }}>
                                    {{ $technique->name }}
                                </option>
                            @endforeach
                        </select>
