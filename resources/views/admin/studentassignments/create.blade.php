...
@section('content')
    <div class="card mt-6">
        <div class="card-header flex flex-row justify-between">
            <h1 class="h6">Student Assignment Toevoegen</h1>
        </div>
        ...
        <div class="card-body grid grid-cols-1 gap-6 lg:grid-cols-1">
            <div class="p-4">
                <form id="form" class="shadow-md rounded-lg px-8 pt-6 pb-8 mb-4" action="{{ route('admin.studentassignments.store') }}" method="POST">
                    @csrf
                    <label class="block text-sm">
                        ...
                        <select name="enrolment_class_id" class="bg-gray-200 block rounded w-full p-2 mt-1 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple form-select" required>
                            @foreach($enrolmentClasses as $enrolmentClass)
                                <option value="{{ $enrolmentClass->id }}">{{ $enrolmentClass->enrolment->student->user->name }} - {{ $enrolmentClass->classYear->schoolClass->name }}</option>
                            @endforeach
                        </select>
                    </label>
                    <label class="block text-sm">
