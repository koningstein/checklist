...
@section('content')
    <div class="card mt-6">
        <div class="card-header flex flex-row justify-between">
            <h1 class="h6">Assignment Verwijderen</h1>
        </div>
        ...
        <div class="card-body grid grid-cols-1 gap-6 lg:grid-cols-1">
            <div class="p-4">
                <form id="form" class="shadow-md rounded-lg px-8 pt-6 pb-8 mb-4"
                      action="{{ route('admin.assignments.destroy', ['assignment' => $assignment->id]) }}" method="POST">
                    @method('DELETE')
                    @csrf
                    ...
                    <label class="block text-sm">
                        ...
                        <input class="bg-gray-200 block rounded w-full p-2 mt-1 focus:border-purple-400
                        focus:outline-none focus:shadow-outline-purple form-input"
                               name="name" value="{{ $assignment->name }}" type="text" disabled/>
                    </label>
                    <label class="block text-sm">
                        ...
                        <textarea
                            ...
                            id="description" rows="7" disabled>{{ $assignment->description }}</textarea>
                    </label>
                    <label class="block text-sm">
                        <span class="text-gray-700">Due Date</span>
                        <input class="bg-gray-200 block rounded w-full p-2 mt-1 focus:border-purple-400
                        focus:outline-none focus:shadow-outline-purple form-input"
                               name="duedate" value="{{ $assignment->duedate }}" type="datetime-local" disabled/>
                    </label>
                    <label class="block text-sm">
                        <span class="text-gray-700">Module</span>
                        <input class="bg-gray-200 block rounded w-full p-2 mt-1 focus:border-purple-400
                        focus:outline-none focus:shadow-outline-purple form-input"
                               name="module_id" value="{{ $assignment->module->name }}" type="text" disabled/>
                    </label>
