...
@section('content')
    <div class="card mt-6">
        <div class="card-header flex flex-row justify-between">
            <h1 class="h6">Assignment Wijzigen</h1>
        </div>
        ...
        <div class="card-body grid grid-cols-1 gap-6 lg:grid-cols-1">
            <div class="p-4">
                <form id="form" class="shadow-md rounded-lg px-8 pt-6 pb-8 mb-4"
                      action="{{ route('admin.assignments.update', ['assignment' => $assignment->id]) }}" method="POST">
                    @method('PUT')
                    @csrf
                    <label class="block text-sm">
                        ...
                        focus:outline-none focus:shadow-outline-purple form-input"
                               name="number" value="{{ old('number', $assignment->number) }}" type="text" required/>
                    </label>
                    <label class="block text-sm">
                        <span class="text-gray-700">Naam</span>
                        <input class="bg-gray-200 block rounded w-full p-2 mt-1 focus:border-purple-400
                        focus:outline-none focus:shadow-outline-purple form-input"
                               name="name" value="{{ old('name', $assignment->name) }}" type="text" required/>
                    </label>
                    <label class="block text-sm">
                        ...
                        <textarea
                            ...
                            id="description" rows="7">{{ old('description', $assignment->description) }}</textarea>
                    </label>
                    <label class="block text-sm">
                    ...
                    <label class="block text-sm">
                        ...
                        <select
                            ...
                            focus:outline-none focus:shadow-outline" name="module_id" id="module_id">
                            @foreach($modules as $module)
                                <option value="{{ $module->id }}"
                                    @selected($module->id == old('module_id', $assignment->module_id))>{{ $module->name }}</option>
                            @endforeach
                        </select>
                    </label>
