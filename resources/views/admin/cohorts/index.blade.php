                ...
                <table class="w-full whitespace-no-wrap">
                    ...
                    @foreach($cohorts as $cohort)
                        <tr class="text-gray-700">
                            <td class="px-4 py-3 text-sm">{{ $cohort->name }}</td>
                            <td class="px-4 py-3 text-sm"><a href="{{ route('admin.cohorts.show', ['cohort' => $cohort->id]) }}">Details</a></td>
                            <td class="px-4 py-3">
                                <div class="flex items-center space-x-4 text-sm">
                                    <a href="{{ route('admin.cohorts.edit', ['cohort' => $cohort->id]) }}">Wijzigen</a>
                                </div>
                            </td>
                            <td>
                                @can('delete cohort')
                                    <div class="flex items-center space-x-4 text-sm">
                                        <a href="{{ route('admin.cohorts.delete', ['cohort' => $cohort->id]) }}">
                                            Verwijderen</a>
                                    </div>
                                @endcan
                            </td>
