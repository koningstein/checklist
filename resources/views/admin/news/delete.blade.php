                ...
                <div class="flex-1 flex items-center justify-center sm:items-stretch sm:justify-start">
                    <div class="sm:block sm:ml-6">
                        <div class="flex space-x-4">
                            <a href="{{ route('admin.news.index') }}" class="text-gray-800 px-3 py-2 rounded-md text-sm font-medium" aria-current="page">Overzicht Nieuws</a>
                            <a href="{{ route('admin.news.create') }}" class="text-gray-800 hover:text-teal-600 transition ease-in-out duration-500 px-3 py-2 rounded-md text-sm font-medium">Nieuws Toevoegen</a>
                        </div>
                    </div>
                </div>
            </div>
...
@section('content')
    <div class="card mt-6">
        <div class="card-header flex flex-row justify-between">
            <h1 class="h6">Nieuwsbericht Verwijderen</h1>
        </div>
        ...
        <div class="card-body grid grid-cols-1 gap-6 lg:grid-cols-1">
            <div class="p-4">
                <form id="form" class="shadow-md rounded-lg px-8 pt-6 pb-8 mb-4"
                      action="{{ route('admin.news.destroy', ['news' => $news->id]) }}" method="POST">
                    @method('DELETE')
                    @csrf
                    <label class="block text-sm">
                        ...
                        focus:outline-none focus:shadow-outline-purple form-input"
                               name="title" value="{{ $news->title }}" type="text" disabled/>
                    </label>
                    <label class="block text-sm">
                    ...
                    <label class="block text-sm">
                        ...
                        <input class="bg-gray-200 block rounded w-full p-2 mt-1 focus:border-purple-400
                        focus:outline-none focus:shadow-outline-purple form-input"
                               name="user_id" value="{{ $news->user->name }}" type="text" disabled/>
                    </label>
