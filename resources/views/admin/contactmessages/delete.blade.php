...
@section('content')
    <div class="card mt-6">
        <div class="card-header flex flex-row justify-between">
            <h1 class="h6">Bericht Verwijderen</h1>
        </div>
        ...
        <div class="card-body grid grid-cols-1 gap-6 lg:grid-cols-1">
            <div class="p-4">
                <form id="form" class="shadow-md rounded-lg px-8 pt-6 pb-8 mb-4"
                      action="{{ route('admin.contactmessages.destroy', ['contactmessage' => $contactmessage->id]) }}" method="POST">
                    @method('DELETE')
                    @csrf
                    ...
                    <label class="block text-sm">
                        ...
                        focus:outline-none focus:shadow-outline-purple form-input"
                               name="name" value="{{ $contactmessage->name }}" type="text" disabled />
                    </label>
                    ...
                    <label class="block text-sm">
                        ...
                        <input class="bg-gray-200 block rounded w-full p-2 mt-1 focus:border-purple-400
                        focus:outline-none focus:shadow-outline-purple form-input"
                               name="email" value="{{ $contactmessage->email }}" type="text" disabled />
                    </label>
                    ...
                    <label class="block text-sm">
                        ...
                        <textarea
                            ...
                            focus:outline-none focus:shadow-outline" name="message" id="message" rows="7" disabled>{{ $contactmessage->message }}</textarea>
                    </label>
                    ...
                    <label class="block text-sm">
                        ...
                        <input class="bg-gray-200 block rounded w-full p-2 mt-1 focus:border-purple-400
                        focus:outline-none focus:shadow-outline-purple form-input"
                               name="status" value="{{ ucfirst($contactmessage->status) }}" type="text" disabled />
                    </label>
