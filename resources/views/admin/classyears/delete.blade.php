...
@section('content')
    <div class="card mt-6">
        <div class="card-header flex flex-row justify-between">
            <h1 class="h6">Class Year Verwijderen</h1>
        </div>
        ...
        <div class="card-body grid grid-cols-1 gap-6 lg:grid-cols-1">
            <div class="p-4">
                <form id="form" class="shadow-md rounded-lg px-8 pt-6 pb-8 mb-4"
                      action="{{ route('admin.classyears.destroy', ['classyear' => $classYear->id]) }}" method="POST">
                    @method('DELETE')
                    @csrf
                    <label class="block text-sm">
                        ...
                        <input class="bg-gray-200 block rounded w-full p-2 mt-1 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple form-input"
                               name="schoolclass_id" value="{{ $classYear->schoolClass->name }}" type="text" disabled/>
                    </label>
                    <label class="block text-sm">
                        <span class="text-gray-700">School Year</span>
                        <input class="bg-gray-200 block rounded w-full p-2 mt-1 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple form-input"
                               name="schoolyear_id" value="{{ $classYear->schoolYear->name }}" type="text" disabled/>
                    </label>
