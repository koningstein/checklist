...
@section('content')
    <div class="card mt-6">
        <div class="card-header flex flex-row justify-between">
            <h1 class="h6">Class Year Wijzigen</h1>
        </div>
        ...
        <div class="card-body grid grid-cols-1 gap-6 lg:grid-cols-1">
            <div class="p-4">
                <form id="form" class="shadow-md rounded-lg px-8 pt-6 pb-8 mb-4"
                      action="{{ route('admin.classyears.update', ['classyear' => $classYear->id]) }}" method="POST">
                    @method('PUT')
                    @csrf
                    <label class="block text-sm">
                        <span class="text-gray-700">School Class</span>
                        <select
                            class="bg-gray-200 block rounded w-full p-2 mt-1 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple form-input"
                            name="school_class_id" required>
                            @foreach($schoolClasses as $schoolClass)
                                <option value="{{ $schoolClass->id }}" @selected($schoolClass->id == old('school_class_id', $classYear->school_class_id))>{{ $schoolClass->name }}</option>
                            @endforeach
                        </select>
                    </label>
                    <label class="block text-sm">
                        ...
                        <select
                            ...
                            @foreach($schoolYears as $schoolYear)
                                <option value="{{ $schoolYear->id }}" @selected($schoolYear->id == old('school_year_id', $classYear->school_year_id))>{{ $schoolYear->name }}</option>
                            @endforeach
                        </select>
                    </label>
