...
@section('content')
    <div class="card mt-6">
        ...
        <div class="card-body grid grid-cols-1 gap-6 lg:grid-cols-1">
            <div class="p-4">
                <form id="form" class="shadow-md rounded-lg px-8 pt-6 pb-8 mb-4" action="{{ route('admin.periods.update', ['period' => $period->id]) }}" method="POST">
                    @method('PUT')
                    @csrf
                    <label class="block text-sm">
                        ...
                        focus:outline-none focus:shadow-outline-purple form-input"
                               name="period" value="{{ old('period', $period->period) }}" type="text" required/>
                    </label>
