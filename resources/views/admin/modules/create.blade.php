...
@section('content')
    <div class="card mt-6">
        <div class="card-header flex flex-row justify-between">
            <h1 class="h6">Module Toevoegen</h1>
        </div>
        ...
        <div class="card-body grid grid-cols-1 gap-6 lg:grid-cols-1">
            <div class="p-4">
                <form id="form" class="shadow-md rounded-lg px-8 pt-6 pb-8 mb-4"
                      action="{{ route('admin.modules.store') }}" method="POST">
                    @csrf
                    ...
                    <label class="block text-sm">
                        ...
                        focus:outline-none focus:shadow-outline-purple form-input"
                            ...
                            @foreach($periods as $period)
                                <option value="{{ $period->id }}" @selected($period->id == old('period_id'))>{{ $period->period }}</option>
                            @endforeach
                        </select>
                    </label>
                    <label class="block text-sm">
                        ...
                        focus:outline-none focus:shadow-outline-purple form-input"
                            ...
                            @foreach($courses as $course)
                                <option value="{{ $course->id }}" @selected($course->id == old('course_id'))>{{ $course->name }}</option>
                            @endforeach
                        </select>
                    </label>
