...
@section('content')
    <div class="card mt-6">
        <div class="card-header flex flex-row justify-between">
            <h1 class="h6">Learning Outcome Toevoegen</h1>
        </div>
        ...
        <div class="card-body grid grid-cols-1 gap-6 lg:grid-cols-1">
            <div class="p-4">
                <form id="form" class="shadow-md rounded-lg px-8 pt-6 pb-8 mb-4" action="{{ route('admin.learningoutcomes.store') }}" method="POST">
                    @csrf
                    <label class="block text-sm">
                        <span class="text-gray-700">Nummer</span>
                        <input type="text" name="number" class="bg-gray-200 block rounded w-full p-2 mt-1 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple form-input" value="{{ old('number') }}" required />
                    </label>
                    <label class="block text-sm">
                        <span class="text-gray-700">Naam</span>
                        <input type="text" name="name" class="bg-gray-200 block rounded w-full p-2 mt-1 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple form-input" value="{{ old('name') }}" required />
                    </label>
                    <label class="block text-sm">
                        <span class="text-gray-700">Beschrijving</span>
                        <textarea name="description" class="bg-gray-200 block rounded w-full p-2 mt-1 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple form-textarea">{{ old('description') }}</textarea>
                    </label>
