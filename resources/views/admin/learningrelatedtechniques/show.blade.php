@extends('layouts.layoutadmin')

@section('topmenu')
    <nav class="card">
        <div class="max-w-6xl mx-auto px-2 sm:px-6 lg:px-8">
            <div class="relative flex items-center justify-between h-16">
                <div class="flex-1 flex items-center justify-center sm:items-stretch sm:justify-start">
                    <div class="sm:block sm:ml-6">
                        <div class="flex space-x-4">
                            <a href="{{ route('admin.learningrelatedtechniques.index') }}" class="text-gray-800 px-3 py-2 rounded-md text-sm font-medium" aria-current="page">Overzicht Learning Related Techniques</a>
                            <a href="{{ route('admin.learningrelatedtechniques.create') }}" class="text-gray-800 hover:text-teal-600 transition ease-in-out duration-500 px-3 py-2 rounded-md text-sm font-medium">Learning Related Technique Toevoegen</a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </nav>
@endsection

@section('content')
    <div class="card mt-6">
        <div class="card-header flex flex-row justify-between">
            <h1 class="h6">Learning Related Technique Details</h1>
        </div>

        <div class="card-body">
            <ul class="list-disc list-inside text-sm text-gray-700">
                <li><strong>ID:</strong> {{ $learningrelatedtechnique->id }}</li>
                <li><strong>Naam:</strong> {{ $learningrelatedtechnique->name }}</li>
                <li><strong>Aangemaakt op:</strong> {{ $learningrelatedtechnique->created_at }}</li>
                <li><strong>Laatst bijgewerkt:</strong> {{ $learningrelatedtechnique->updated_at }}</li>
            </ul>
        </div>
    </div>
@endsection
