...
@section('content')
    <div class="card mt-6">
        <div class="card-header flex flex-row justify-between">
            <h1 class="h6">Learning Suboutcome Level Wijzigen</h1>
        </div>
        ...
        <div class="card-body grid grid-cols-1 gap-6 lg:grid-cols-1">
            <div class="p-4">
                <form id="form" class="shadow-md rounded-lg px-8 pt-6 pb-8 mb-4" action="{{ route('admin.learningsuboutcomelevels.update', ['learningsuboutcomelevel' => $learningSuboutcomeLevel->id]) }}" method="POST">
                    @method('PUT')
                    @csrf
                    <label class="block text-sm">
                        ...
                        <select name="learning_suboutcome_id" class="bg-gray-200 block rounded w-full p-2 mt-1 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple form-select" required>
                            @foreach($learningSuboutcomes as $learningSuboutcome)
                                <option value="{{ $learningSuboutcome->id }}" {{ $learningSuboutcomeLevel->learning_suboutcome_id == $learningSuboutcome->id ? 'selected' : '' }}>
                                    {{ $learningSuboutcome->name }}
                                </option>
                            @endforeach
                        </select>
                    </label>
                    <label class="block text-sm">
                        ...
                        <select name="learning_level_id" class="bg-gray-200 block rounded w-full p-2 mt-1 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple form-select" required>
                            @foreach($learningLevels as $learningLevel)
                                <option value="{{ $learningLevel->id }}" {{ $learningSuboutcomeLevel->learning_level_id == $learningLevel->id ? 'selected' : '' }}>
                                    {{ $learningLevel->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <label class="block text-sm">
                        ...
                        <textarea name="description" class="bg-gray-200 block rounded w-full p-2 mt-1 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple form-textarea" required>{{ old('description', $learningSuboutcomeLevel->description) }}</textarea>
                    </label>
